...
```{r}
# Evaluate ELO history for every fighter 
elo_history %>% 
  filter(k %in% c(25, 30, 35)) %>% 
  group_by(k) %>% 
  mutate(pk = row_number()) %>% 
  arrange(team, pk) %>% 
  group_by(team, k) %>% 
  mutate(fights = row_number()) %>% 
  ungroup() %>% 
  ggplot(aes(x = fights, y = elo)) + 
  geom_line(aes(group = team, color = as.factor(k)), alpha = .2) + 
  geom_smooth() + 
  facet_wrap(~k, scales = "fixed")
```

Higher K seems to be beneifical for veterans and overrates them.  
Model will use a lower k of 25.  
...
final_elo_model <- elo.run(res~fighter + opponent + regress(start_regress, 1500, .5), data = elo_df, k = 25)
