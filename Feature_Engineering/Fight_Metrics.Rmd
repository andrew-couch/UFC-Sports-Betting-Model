...
fight_table <- inner_join(fighter_table, rename_all(fighter_table, .funs = ~paste0("opp_", .x)), 
  ...
  mutate(submission_metric = log((avg_td_landed * (opp_loss_submission + 1) * (win_submission + 1)) + 1) + 
           log((avg_tds_defended + 1) / (loss_submission + 2)),
         opp_submission_metric = log((opp_avg_td_landed * (loss_submission + 1) * (opp_win_submission + 1)) + 1) + 
           log((opp_avg_tds_defended + 1) / (opp_loss_submission + 2))) %>% 
  mutate(sub_dif = submission_metric - opp_submission_metric)
```

```{r}
# Export tables 
write_csv(fight_table, "E:/School/R Work/UFC-Sports-Betting-Model/Data/fights.csv")
write_csv(fighter_table, "E:/School/R Work/UFC-Sports-Betting-Model/Data/fighter_table.csv")
```
