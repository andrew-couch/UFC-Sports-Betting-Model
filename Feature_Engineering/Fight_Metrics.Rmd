...
fighter_table <- df %>% 
  ...
  select(-round_finished, -rounds, -time, -date, -last_fight_date) %>%
...
# Randomly sample each fight (randomly chooses who is the fighter and who is the opponent)
fight_table <- fight_table %>% 
  group_by(fight_pk) %>% 
  sample_n(size = 1) %>% 
  ungroup() %>% 
